...
class ItemControllerTest {
    ...
    //로그인된 사용자의 Role에 따라 상품등록 페이지에 접근이 가능한지 테스트
    @Test
    @DisplayName("상품 등록 페이지 권한 테스트")
    @WithMockUser(username = "admin", roles = "ADMIN")  //현재 회원의 이름이 admin이고, role이 ADMIN인 유저가 로그인된 상태로 테스트를 할 수 있도록 해주는 어노테이션
    public void itemFormTest() throws Exception{
        mockMvc.perform(MockMvcRequestBuilders.get("/admin/item/new"))  //상품 등록 페이지에 요청을 get요청을 보낸다
                .andDo(print()) //요청과 응답 메시지를 확인할 수 있또록 콘솔창에 출력
                .andExpect(status().isOk());    //응답 상태 코드가 정상인지 확인
    }

    //로그인된 사용자의 Role이 USER면 상품등록 페이지에 접근이 안되는지 테스트
    @Test
